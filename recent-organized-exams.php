...
$recent_query="SELECT * FROM `question_files` WHERE `user_id`='$login_photo'";
    $recent_run=mysqli_query($con,$recent_query);

    if(mysqli_num_rows($recent_run) >= 1){
      while($recent_data=mysqli_fetch_assoc($recent_run))
      {
  
        ?>
        <a href="preview?id=<?php echo $recent_data['unique_exam_id'];?>">
  <div class="w3-container w3-card w3-margin" style="width:200px;height:200px">
  <h3 id="examsss"><?php echo $recent_data['exam_count'];?></h3>
  <h3><?php echo $recent_data['course'];?></h3>
  <a href="invite?id=<?php echo $recent_data['unique_exam_id'];?>" class="w3-btn w3-blue w3-text-white w3-card w3-round">Invite</a>
  <button onclick="myFunction()" class="w3-btn w3-red w3-text-white w3-card w3-round">Delete</button>
  </div>
  </a>
        <?php
  ?>
  <div class="w3-container">
      <div class="w3-modal" id="delete_exam"  style="display:none">
      <div class="w3-modal-content w3-card-4 w3-animate-right w3-round-large">
      <div class="w3-center">
      
      <h3>Do you want to delete exam?</h3>    
      </div>
      <div class="w3-container w3-center">
      <form action="delete_exam" method="post">
      <input type="text" value="<?php echo $recent_data['file_id']?>" name="file_id" style="display:none">
      <input type="submit" class="w3-btn w3-red w3-margin w3-round w3-card" style="width:200px">
      </form>
      <a class="w3-btn w3-blue w3-margin w3-round w3-card" style="width:200px" onclick="delete_cut()" name="submit_name">Cancel</a>
      </div>
      </div>
      </div>
    <?php
...
function myFunction() {
  var x = document.getElementById("delete_exam");
