...
$course_query="SELECT * FROM `exam_course`";
$course_run=mysqli_query($con,$course_query);
?><div style="overflow:auto;display:flex;max-height:250px;margin:10px" class="scrolling w3-border w3-round-large"><?php

if(mysqli_num_rows($course_run)>0)
{
while($course_data=mysqli_fetch_assoc($course_run))
{
  $color1=rand(0,255);
  $color2=rand(0,255);
  $color3=rand(0,255);
  ?><a href="choose_course" style="text-decoration:none"><div class="w3-card w3-margin w3-text-white" style="min-width:200px;height:150px;line-height:140px;text-align:center;background:rgb(<?php echo $color1;?>,<?php echo $color2;?>,<?php echo $color3;?>)"><h1><?php echo $course_data['course'];?></h1></div></a><?php
}
}
else
{
  echo "<h3>No Exams Found!</h3>";
}
?></div><?php
