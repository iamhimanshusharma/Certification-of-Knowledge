...
$q_query="SELECT * FROM `filed_questions` WHERE `user_id`='$login_photo' AND `unique_exam_id`='$unique_id'";
$q_run=mysqli_query($con,$q_query);

$q_data=mysqli_fetch_assoc($q_run);

?><h3>Exam: <?php echo $q_data['exam_count']?> | Course: <?php echo $q_data['exam_course']?></h3><?php



if(mysqli_num_rows($q_run)>0)
{
  $q_count=mysqli_num_rows($q_run);
for($i=0;$i<$q_count;$i++)
  {
    $question_query="SELECT * FROM `filed_questions` WHERE `user_id`='$login_photo' AND `unique_exam_id`='$unique_id' AND `question_id`='$i'";
    $question_run=mysqli_query($con,$question_query);

    $question_data=mysqli_fetch_assoc($question_run);

    ?><h2><?php echo $question_data['question'];?></h2><?php

    $option_query="SELECT * FROM `filed_options` WHERE `user_id`='$login_photo' AND `unique_exam_id`='$unique_id' AND `question_id`='$i'";
    $option_run=mysqli_query($con,$option_query);

    while($option_data=mysqli_fetch_assoc($option_run))
{
  ?><h4><li><?php echo $option_data['options'];?></li></h4><?php
}

$answer_query="SELECT * FROM `filed_answers`  WHERE `user_id`='$login_photo' AND `unique_exam_id`='$unique_id' AND `question_id`='$i'";
$answer_run=mysqli_query($con,$answer_query);

$answer_data=mysqli_fetch_assoc($answer_run);
$opt = $answer_data['answer_id'];

$o_query="SELECT * FROM `filed_options` WHERE `user_id`='$login_photo' AND `unique_exam_id`='$unique_id' AND `question_id`='$i' AND `options_id`='$opt'";
$o_run=mysqli_query($con,$o_query);

$o_data=mysqli_fetch_assoc($o_run);

?>
<h5 class="w3-text-green">
<?php echo "Answer is: ".$o_data['options'];?>
</h5>
<hr>
<?php
  }
